...
const updateData = async () => {
    try {
        if(Data.title === "" || Data.desc === "" ) {
            ...
            const res = await axios.put(`https://expressbackend-one.vercel.app/api/v2/update-tasks/${editData.id}`, Data, {headers});
            console.log(res)
            setData({title: "", desc: ""})
            setInputDiv('hidden')
        }
...
const submit = async () => {
    try {
        if(Data.title === "" || Data.desc === "" ) {
            ...
            const res = await axios.post(`https://expressbackend-one.vercel.app/api/v2/create-task`, Data, {headers});
            console.log(res)
            setData({title: "", desc: ""})
            setInputDiv('hidden')
        }
